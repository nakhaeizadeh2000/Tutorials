...
- Done:
  ...
  - [unit D7-9] Created leaf `07/sections/6. Overlaps to avoid/6.1. Boundaries what is covered elsewhere.md` (underneath → 01–06, platform edge, words → ITVocabulary; track-complete map)
  - [unit D7-10] Final forward-link retirement sweep: 40 replacements across 34 files via scripted exact-match pass (`/tmp/git_forward_sweep.py`, 40/40 applied, 0 misses) — every remaining `(planned, see LOG)` forward outside LOG history → live link (domain indexes where general, specific leaves where precise: bisect → 07/3.1, stash → 06/2.2, lease → 05/2.1, rebase → 05/2.2, revert → 05/1.2, amend → 05/2.1, signing → 07/1.1, secrets → 07/1.2, merge policy → 06/1.2, strategy → 06/1.1, changelogs → 06/3.1, PR flow → 04/2.2, fetch/push → 04/1.1, tracking → 04/1.2, currency → 04/3.1, staging → 02/1.1, messages → 02/1.2, ignore → 02/3.1, rerere → 03/3.1); code-fence comments reworded to live-link-below; 6.1 maintenance notes reworded to past tense (history, not placeholders)
- Files touched:
...
- Verification:
  ...
  - [unit D7-10] link checker over whole Git track (66 md files): 459 angle-bracket targets, 5 broken — all 5 backticked doc-text in LOG history (same class, unchanged); real content 454/454 resolve, 0 `%20`; zero `(planned, see LOG)` outside LOG history (grep-confirmed); filenames clean (0 bad-char hits); Domain 07 promises 8/8 ↔ leaves 8/8; heading prefixes 8/8 match; track README 7/7 rows resolve
  - TRACK-COMPLETE DoD walk (§4): all 7 domain indexes Template B with 8/8 promise↔leaf delivery; every leaf junior-readable first half (plain definitions + verified runnable commands) / expert-mentor second half (trade-offs, performance, mentor notes); every anti-pattern labeled with why; all git-command claims executed on Git 2.53.0 before writing (fetch/push refusal rc=1, --ff-only fatal rc=128, ahead1/behind1, -u pairing, no-upstream fatal, two-remote fetch --all, amend hash replacement, revert/-m 1 chains, rebase/cherry-pick outputs, reflog rescue count-0-to-restored, tag/describe, stash round-trip, bisect session, verify-commit rc=1, secret shape-check trip); forge/tooling claims (`gh`, protection, queues, semantic-release, GPG setup) labeled documented-per-docs with git-side steps verified; zero duplication (DRY boundaries per domain, single-source-of-truth preserved); formatting matches neighbors (bold labels, angle-bracket literal-space links, `---` separators, `bash` fences); 40 commits this session (1 log-open + 7 indexes/link-sets + 32 leaves/sweeps), tree clean.
- Next steps: TRACK COMPLETE — all 7 domains implemented (01 Fundamentals, 02 Workflow, 03 Branching, 04 Remotes, 05 Undo/Rewrite, 06 Workflows/Releases, 07 Production), track README 7/7 rows, 66 md files, 454/454 content links resolve. Known-accepted remainders: (a) 5 backticked doc-text matches in LOG history read as broken by naive checkers (pre-existing class, not real links); (b) `/tmp/git_forward_sweep.py` + `/tmp/g05` `/tmp/g07` scratch dirs live outside the repo (intentional — not committed). Suggested next work (new session/block only if requested): start the next track per root README mentor-path order (first unimplemented: #5 Algorithm Design); refresh Git leaves when Git 2.56+ or forge-rule changes land.
