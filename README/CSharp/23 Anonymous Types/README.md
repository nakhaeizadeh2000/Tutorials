...
1. **`var` introduction + basic anonymous-type mention** is covered under `README/CSharp/17 Extension Methods and others/sections/4. Implicitly typed variables var/4.1. var rules legality inference anonymous types`
2. **General LINQ fundamentals** (enumeration/projection mechanics) live in `README/CSharp/21 Collections`
3. **Runtime allocation/GC deep theory** lives in `README/CSharp/01 Fundamental Theories` and `README/CSharp/18 GC and Destructors and IDisposable`
