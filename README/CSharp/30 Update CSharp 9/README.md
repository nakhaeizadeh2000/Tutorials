...
### [1.1. Top-level statements (what the compiler generates, trade-offs)](<../02 CSharp language basics/sections/1. Getting started/1.2. Creating your first app (Console + top-level statements).md>)
...
### [1.2. Command line arguments (`args`), `Environment`, exit codes, parsing](<../02 CSharp language basics/sections/1. Getting started/1.3. Command line arguments args Environment exit codes parsing.md>)
...
### [2.1. File-scoped namespaces (`namespace X;`)](<../11 Namespaces/sections/2. Creating namespaces/2.2. File-scoped namespaces modern default.md>)
...
### [2.2. Global using (project-wide imports)](<../11 Namespaces/sections/4. Importing namespaces/4.2. Global using project wide imports.md>)
...
### [3.1. Nullable reference types (NRT)](<../16 Handling Null/sections/2. Nullable reference types/2.1. Enabling NRT and reading annotations.md>)
...
### [3.2. Null-forgiving operator (`!`) — last resort](<../16 Handling Null/sections/4. Operators and constructs/4.3. Null-forgiving operator last resort.md>)
...
### [4.1. Target-typed `new` (`new()`) and constructor inference](<../17 Extension Methods and others/sections/4. Implicitly typed variables var/4.3. Target-typed new constructor inference.md>)
...
### [5.1. `switch` statements and `switch` expressions (modern C#)](<../02 CSharp language basics/sections/5. Branching and decision making/5.3. switch and switch expressions modern CSharp.md>)
...
### [5.2. Pattern matching — type pattern (and safe casts)](<../06 Type Conversion/sections/3. Explicit conversions and casts/3.2. Downcasts and safe casting patterns is pattern matching as.md>)
...
### [5.3. Pattern matching — null patterns and flow analysis helpers](<../16 Handling Null/sections/2. Nullable reference types/2.2. Flow analysis what the compiler can prove.md>)
...
### [5.4. Pattern matching — tuple pattern](<../24 Tuples/sections/4. Deconstructing/4.3 Deconstructing plus pattern matching tuple patterns.md>)
...
### [5.5–5.6. Pattern matching — relational, logical, property, positional, list, slice, `var` (catalog)](<../02 CSharp language basics/sections/5. Branching and decision making/5.4. Pattern matching catalog type relational property list slice var (CSharp 9 through 15).md>)
...
3. **Also see**: [5.3 `switch` expressions](<../02 CSharp language basics/sections/5. Branching and decision making/5.3. switch and switch expressions modern CSharp.md>), `Handling Null`, `Tuples`, [Update C# 11](<../32 Update CSharp 11/README.md>) for list/slice patterns

## 6. Immutability as a design tool (correctness + concurrency + versioning)

### [6.0. Need of immutability + immutable classes (reasoning + API design)](<../03 CSharp Basic Object Oriented Programming/sections/3. Important points about objects/3.1. Reference semantics aliasing mutation defensive design.md>)
...
4. **Also read**: [choosing class vs struct vs record vs record struct](<../03 CSharp Basic Object Oriented Programming/sections/2. Creating classes and objects/2.4. Choosing class vs struct vs record vs record struct.md>) (immutability-friendly shapes)

### [6.1. `init` / `required` and “make illegal states unrepresentable”](<../07 Constructors/sections/4. Object initialization styles/4.2. init required and illegal states unrepresentable.md>)
...
### [6.2. Readonly fields (immutability foundations)](<../04 Fields/sections/6. Readonly fields/6.1. Readonly instance fields immutability and invariants.md>)
...
### [6.3. Read-only properties + `init` (immutability ergonomics)](<../08 Properties and Indexers/sections/3. Read-only and write-only properties/3.1. Read-only properties get-only private set immutability.md>)
...
### [7.1. Readonly structs (guarantees, defensive copies)](<../13 Structures/sections/4. Readonly structs/4.1. readonly struct guarantees and limits.md>)
...
### [7.2. Parameterless struct constructors + `default(T)` (modern rules)](<../13 Structures/sections/2. Constructors in structs/2.1. Struct constructors rules.md>)

1. **Parameterless struct ctors** do not remove **`default(T)`**—design for both paths
2. **Also read**: [auto-default / contract framing](<../13 Structures/sections/1. Structures fundamentals/1.4. Auto default structs default T vs parameterless constructors.md>)
...
### [8.1. Records: generated behaviors, equality, `with`, printing](<../07 Constructors/sections/2. Constructor kinds/2.3. Records and constructor generated members.md>)
...
### [8.2. Records and equality details (`Equals`, `GetHashCode`, `ToString`)](<../14 System Object Class/sections/3. Overriding correctly/3.3. Records and generated equality.md>)
...
1. **Core behavior + `with`**: [Constructors — records](<../07 Constructors/sections/2. Constructor kinds/2.3. Records and constructor generated members.md>)
2. **Equality / `ToString`**: [System.Object — records](<../14 System Object Class/sections/3. Overriding correctly/3.3. Records and generated equality.md>)
3. **Shapes**: [class vs struct vs record](<../03 CSharp Basic Object Oriented Programming/sections/2. Creating classes and objects/2.4. Choosing class vs struct vs record vs record struct.md>); inheritance/`sealed` follow normal type rules—keep record hierarchies shallow

## 9. Partial methods and advanced compilation hooks

### [9.1. Partial methods: return values, accessibility, attributes](<../12 Partial and Static Classes, Enumerations/sections/2. Partial methods/2.2. Partial methods modern rules accessibility return attributes.md>)
...
### [9.2. Module initializers (`[ModuleInitializer]`)](<../12 Partial and Static Classes, Enumerations/sections/3. Static classes/3.4. Module initializers ModuleInitializer attribute trimming and AOT.md>)
...
### [10.1. `static` lambdas (avoid captures/closures)](<../19 Delegates and Events/sections/7. Anonymous methods and lambdas/7.4. static lambdas prevent captures.md>)
...
### [10.2. Lambda return type, defaults, modifiers (C# 10+)](<../19 Delegates and Events/sections/7. Anonymous methods and lambdas/7.5. Lambda explicit return type default parameters modifiers (CSharp 10 through 15).md>)
...
### [11.1. Interpolated strings (modern formatting guidance)](<../26 String and DateTime and Math/sections/5. Formatting Strings/5.1. Composite formatting vs interpolated strings and modern handler guidance.md>)
...
### [11.2. Constant interpolated strings (C# 10+)](<../26 String and DateTime and Math/sections/5. Formatting Strings/5.5. Constant interpolated strings compile time const contexts.md>)

1. **`const string`** when all holes are compile-time constant-compatible
2. **Not** runtime formatting—no culture at compile time
3. **See also**: [Update C# 10](<../31 Update CSharp 10/README.md>)

## 12. Interfaces (default implementations and modern capabilities)

### [12.1. Default interface methods (DIM) — when they compete with extensions](<../17 Extension Methods and others/sections/1. Extension methods language rules/1.5. Extension methods vs default interface methods.md>)
...
### [12.2. Default interface members (DIM) — language semantics and pitfalls](<../10 Abstract Classes and Interfaces/sections/3. Interfaces/3.2. Default interface members DIM.md>)
...
### [12.3. Static abstract members in interfaces (generic math / type-class style)](<../10 Abstract Classes and Interfaces/sections/3. Interfaces/3.3. Static abstract members in interfaces generic math.md>)
...
### [12.4. Private interface methods and interface member forms (overview)](<../10 Abstract Classes and Interfaces/sections/3. Interfaces/3.4. Interface member forms private static abstract sealed DIM overview.md>)

1. **Private helpers** on interfaces factor default implementations without widening API
2. **Static abstract**, DIM, overrides: see [3.2](<../10 Abstract Classes and Interfaces/sections/3. Interfaces/3.2. Default interface members DIM.md>) / [3.3](<../10 Abstract Classes and Interfaces/sections/3. Interfaces/3.3. Static abstract members in interfaces generic math.md>)
3. **Perf**: interface dispatch in hot loops—measure

## 13. Indexing and slicing primitives

### [13.1–13.2. Index-from-end (`^`), `Range` (`..`), `Span` / `ReadOnlySpan`](<../20 Arrays/sections/1. Arrays fundamentals/1.5. Index from end Range and slicing with Span.md>)
