...
### [1.4. Auto-default structs and the `default(T)` contract](<./sections/1. Structures fundamentals/1.4. Auto default structs default T vs parameterless constructors.md>)

1. **`default(T)` always exists**; parameterless struct constructors do not remove it
2. **Decide** if all-zero is valid; validate at boundaries if not ([2.1](<./sections/2. Constructors in structs/2.1. Struct constructors rules.md>))
3. **Version map**: [Update C# 11](<../32 Update CSharp 11/README.md>)
...
2. **`in` parameters** (when they help; when they hurt by forcing copies/—measure)
...
### [4.3. `ref` fields (C# 11+)](<./sections/4. Readonly structs/4.3. ref fields scoped ref safety and CSharp 11.md>)

1. **In `ref struct`** for high-performance views; obey **ref safety** (`scoped`, lifetimes)
2. **Generics**: `allows ref struct` (C# 13) - see `Generics` domain
3. **Version map**: [Update C# 11](<../32 Update CSharp 11/README.md>)
