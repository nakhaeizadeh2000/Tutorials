...
1. **LINQ general iterator/IEnumerable fundamentals** overlap with `README/CSharp/Collections` (enumeration, iterator blocks, deferred execution framing); this domain focuses on the operator behaviors you listed (ordering, element access, projection, and aggregations).
2. **Lambda/closure basics** overlap with `README/CSharp/Delegates and Events` and `README/CSharp/Extension Methods and others`; here it's only referenced where it impacts LINQ allocations and correctness.
3. **Projection into anonymous types/tuples** overlaps with `README/CSharp/Anonymous Types` and `README/CSharp/Tuples`; this domain treats projection as "shape transformation" but doesn't re-explain those specific language constructs.
