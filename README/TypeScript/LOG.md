...
- Decisions: kept single-dot as in domain README (canonical); removed extraneous `.` that broke `find`/`link` resolution.
- Files touched: renamed 1 leaf file (06/2.1) via `git mv`; modified `README/TypeScript/LOG.md`.
- Links fixed / added: `06/README.md:27` now resolves (verified `ls` shows file).
- Verification: `ls -l "README/TypeScript/06 .../2. Mapped, Template Literal and Key Remapping/"` shows 3 leaf files with correct names; `find ... -name "2.1. Mapped*"` resolves.
- Next steps: unit 2 — flatten 08/2.2 nested leaf.
