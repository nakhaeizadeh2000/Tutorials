...
- Decisions: folder rename preserves `M. ` prefix and spaces, only removes `,` to meet §8 sanitization; CSharp has 0 commas — now TS matches. Used `git mv` for history. Bulk link fix via python pairs for both raw and encoded forms.
- Files touched: renamed 6 directories via `git mv`; modified 26 files (5 READMEs: `02`, `03`×2, `04`, `06`; 21 leaves + `LOG.md`): list includes `02 README`, `03 README×2`, `04 README`, `06 README`, plus leaves `02 1.1/1.2/3.2`, `03 2.1/3.1/3.2`, `04 1.1/1.2`, `05 2.2`, `06 1.2/3.1`, `01 1.3/2.1/2.2`, `11 1.2/2.2`, `10 1.2/2.1/3.1`, `07 2.2`, `08 1.2` and `LOG.md`.
- Links fixed / added: all section links now use comma-free paths (e.g., `./sections/2. Unions Intersections and Narrowing/2.1...`), cross-links encode without `%2C`; verified `find -type d -name "*,*"` returns 0 and `grep -r "%2C"` returns 0 in TS track.
- Verification: `find README/TypeScript -type d -name "*,*" | wc -l` = 0, `ls -R` shows 6 renamed folders, `find .../sections/*/*.md | wc -l` = 62 (leaves moved with dirs), `grep -r "IsolatedModules,.*SWC"` 0, domain README links `grep -c "<./sections"` 11× OK.
- Next steps: unit 5 — normalize leaf style (add --- separators, de-orphan headings).
